...
yt = transpose(y)
ap_mask = @. (abs(x - X/2) < ap.x/2) * (abs(yt - Y/2) < ap.y/2)
...
@progress for i in 1:10000
    p1 = sum(abs2.(u))
    elapsed_cpu = @elapsed free_propagate!(u, trans, plan, iplan); u .*= ap_mask
    elapsed_gpu = CUDA.@elapsed free_propagate!(u_d, trans_d, plan_d, iplan_d); u_d .*= ap_mask_d 
